    ...
    async def _handle_external_auth_for_refresh(self, request: Request) -> Response:
        ...
        try:
            ...
            try:
                ...
                user, tokens = await verify_external_auth(request=request, db=session)
                ...
                try:
                    ...
                    logger.error(f"Error closing session in refresh handler: {str(e)}")
        except Exception as e:
        ...
        if "access_token_lf" not in request.cookies and "asc_auth_key" in request.cookies:
            ...
            try:
                ...
                try:
                    ...
                    user, tokens = await verify_external_auth(request, session)
                    
                    # If we have tokens from external auth, set cookies
                    if tokens:
                        await set_auth_cookies(response, tokens)
                except Exception as e:
                    # Log the error but don't interrupt the request flow
                    from loguru import logger
                    logger.error(f"Error in external auth middleware: {str(e)}")
                finally:
                    # Make sure to close the session
                    try:
                        await session.close()
                        ...
                        from loguru import logger
                        logger.error(f"Error closing session in middleware: {str(e)}")
            except Exception as e:
                from loguru import logger
